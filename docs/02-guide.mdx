...
```bash
cargo run --bin db-peace-cli create-peace-user -u postgresql://postgres:123456@localhost:5432/peace --username test --password test --email test@email.com
```
...
cargo run --bin gateway-server -- --log-env-filter=tower_http=debug --tls --ssl-cert=./cert.cer --ssl-key=./key.pem --force-https --admin-endpoints --bancho-rpc-uri http://127.0.0.1:50051 --bancho-rpc-lazy-connect --bancho-state-rpc-lazy-connect
```
