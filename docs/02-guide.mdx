...
cargo run --bin bancho-state-server -- --rpc-reflection --rpc-admin-endpoints --bancho-state-save-dump --bancho-state-load-dump
```
...
2023-07-24T15:17:54.155786Z  INFO peace_services::bancho_state::services::bancho_state: [BanchoStateDump] Dump file not found, path: "./.service_dump/bancho_state.dump"
...
The `--bancho-state-save-dump` parameter allows the Bancho state service to store
the service instance in memory to the hard disk when it terminates.

By passing the `--bancho-state-load-dump` parameter at startup, the service can be restored to the state it was in when it was last closed,
without losing user sessions, and users will not feel that the service has restarted.

It is worth noting that the `--bancho-state-dump-expries` parameter can control the expiration time (in seconds) of the snapshot.
...
cargo run --bin chat-server -- --peace-db-url sqlite://peace.db --rpc-reflection --rpc-admin-endpoints --chat-save-dump --chat-load-dump
```
...
>> Log level: Info (debug_mode: false)
>> Runtime: single thread
2023-07-24T15:19:16.783822Z  INFO peace_services::chat::services::chat: [ChatDump] Dump file not found, path: "./.service_dump/chat.dump"
...
:::tip
The `--chat-save-dump` parameter allows the Chat service to store
the service instance in memory to the hard disk when it terminates.

By passing the `--chat-load-dump` parameter at startup, the service can be restored to the state it was in when it was last closed,
without losing user sessions, and users will not feel that the service has restarted.

It is worth noting that the `--chat-dump-expries` parameter can control the expiration time (in seconds) of the snapshot.
If the snapshot expires, it will not be loaded.
:::
